...
(define (operator exp) (car exp)) (define (operands exp) (cdr exp))
...
(define (setup-environment) (let ((initial-env (extend-environment (primitive-procedure-names) (primitive-procedure-objects) the-empty-environment)))
...
(define (apply-primitive-procedure proc args)
  (apply-in-underlying-schema (primitive-implementation proc) args))

(define apply-in-underlying-schema apply)
(define (apply2 procedure arguments)
  (cond ((primitive-procedure? procedure)
        ...
        (else (error "Unkonwn procedure type: APPLY" procedure))))
...
(driver-loop)
; (display (eval '(+ 2 3) the-global-environment))